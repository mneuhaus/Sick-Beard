...
def getAllLanguages ():
    ...
    sqlLanguages = myDB.select("SELECT DISTINCT(t.audio_langs), t.lang FROM tv_shows t, tv_episodes e WHERE t.tvdb_id = e.showid AND (e.status = ? OR e.status = ? OR e.status = ?)", [common.UNAIRED,common.WANTED,common.UNSATISFIED])
    
    languages = []

    for language in sqlLanguages:
        for language2 in str(language["audio_langs"]).split("|"):
            if language2.split(":")[0] == u"tvdb":
                languages.append(str(language["lang"]))
            else:
                languages.append(language2.split(":")[0])
